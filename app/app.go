...
type home struct {
	...
	program  string
	autoYes  bool
	// repoPath is the default path used when creating new instances.
	// If not a git repo, new instances will use NoWorktree mode.
	repoPath string
...
func (m *home) handleKeyPress(msg tea.KeyMsg) (mod tea.Model, cmd tea.Cmd) {
	...
	if m.state == stateNew {
		...
		case tea.KeyEnter:
			...
			// "claude" is a reserved name for a no-worktree scratch instance.
			// runs clauded (claude --dangerously-skip-permissions) directly in
			// the project directory with no branch isolation.
			if instance.Title == "claude" {
				instance.SetNoWorktree(true)
				instance.Program = "clauded"
			}
	...
	case keys.KeyPrompt:
		...
		instance, err := session.NewInstance(session.InstanceOptions{
			...
			Path:       m.repoPath,
			Program:    m.program,
			NoWorktree: !git.IsGitRepo(m.repoPath),
		})
		if err != nil {
			return m, m.handleError(err)
		}
